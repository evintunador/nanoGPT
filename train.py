...
$ python train.py --config=config/default.yaml --batch_size=32 --compile=False
...
$ torchrun --nproc_per_node=8 --nnodes=2 --node_rank=0 --master_addr=123.456.123.456 --master_port=1234 train.py --config=config/default.yaml
- Run on the worker node:
$ torchrun --nproc_per_node=8 --nnodes=2 --node_rank=1 --master_addr=123.456.123.456 --master_port=1234 train.py --config=config/default.yaml
...
def main():
    # Setup argument parser and load configuration
    parser = argparse.ArgumentParser(description='Train a GPT model')
    config = compose_config(parser)
    
    # Add timestamp subdirectory to output directory for unique runs
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_out_dir = config.out_dir
    config.out_dir = os.path.join(base_out_dir, timestamp)
    
    # Handle dtype default logic (depends on CUDA availability)
    if 'dtype' not in config or config.dtype is None:
    ...
    with DistributedManager() as dist:
        ...
        device = dist.device
        master_process = dist.is_main_process
        ddp_world_size = dist.world_size
        ddp_local_rank = dist.local_rank
        ...
        with ReproducibilityManager(output_dir=config.out_dir, is_main_process=master_process) as rm:
            ...
            if config.init_from == 'scratch':
                ...
                checkpoint_model_args = meta.get('model_args')
                ...
                # Now load state into the model
                load_checkpoint(ckpt_path, model=model)
                
                iter_num = meta.get('iter_num', 0)
                best_val_loss = meta.get('best_val_loss', 1e9)
            ...
            if config.init_from == 'resume':
                ckpt_path = os.path.join(config.out_dir, 'ckpt.pt')
                load_checkpoint(ckpt_path, optimizer=optimizer)
                    ...
                    if losses['val'] < best_val_loss or config.always_save_checkpoint:
                        ...
                        if iter_num > 0:
                            ...
                            repro_metadata = {
                                ...
                                "software_env": rm.software_environment,
                                "runtime_env": rm.runtime_environment,
                                "rng_state": rm.get_rng_states(),
                            }
                            
                            save_checkpoint(
                                filepath=os.path.join(config.out_dir, 'ckpt.pt'),
                                metadata={
                                    'iter_num': iter_num,
